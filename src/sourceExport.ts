...
export type SourceFormat = 'c' | 'rust' | 'z80' | '6502' | '68000' | 'x86'
...
export function exportSource(font: FontInstance, format: SourceFormat): string {
  ...
  if (format === 'c') {
    ...
    for (let g = 0; g < count; g++) {
      const bytes = Array.from(data.subarray(g * bpg, (g + 1) * bpg))
      lines.push(`\t.byte ${bytes.join(',')} ; ${charComment(startChar + g)}`)
    }
    ...
    for (let g = 0; g < count; g++) {
      const bytes = Array.from(data.subarray(g * bpg, (g + 1) * bpg))
      lines.push(`\tDC.B ${bytes.join(',')} ; ${charComment(startChar + g)}`)
    }
    ...
    for (let g = 0; g < count; g++) {
      const bytes = Array.from(data.subarray(g * bpg, (g + 1) * bpg))
      lines.push(`\tdb\t${bytes.join(',')} ; ${charComment(startChar + g)}`)
    }
  }
