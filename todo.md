...
- [x] **Input validation in simpler parsers** — `drawParser.ts` assumes uniform glyph size, `bbcParser.ts` doesn't validate char code range.
...
- [x] **`useClickOutside` refs dependency** — `useClickOutside.ts:26` may cause unnecessary listener re-registration when inline arrays are passed.
- [x] **PreviewPane is 496 lines** — could extract ColorSwatch and text selection logic to separate files.
