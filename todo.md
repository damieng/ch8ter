...
- [ ] **Truncated last glyph silently dropped** — `store.ts:91`: if `fontData.length % bpg !== 0`, the last partial glyph is silently discarded. Should pad or warn.
...
- [ ] **BasePane useEffect missing `windowId` dependency** — `BasePane.tsx:102` has `[aspectRatio]` but handler uses `windowId`. Stale closure risk.
